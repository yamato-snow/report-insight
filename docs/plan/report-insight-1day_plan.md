...
- [基本設計書](../02_basic_design.md)（構成図・処理フロー・アプリ構成 §7 のディレクトリに従う）
...
| 夕方② | Terraform 一式(modules 5種+envs/dev), GitHub Actions 仕上げ, README のセットアップ手順・ステータス更新 | P1 |
...
- [ ] GitHub Actions: lint→test は毎PR、LLM回帰評価は `app/llm/prompts/**` 変更時のみ、main で build まで定義済み
