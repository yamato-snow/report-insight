...
| ID | 受入条件 |
|---|---|
| F-4-1 | 支店管理者は自支店の報告書のみを一覧でき、物件・事象分類・緊急度・ステータスで絞り込める。品質管理部は全支店を横断できる。権限外の報告書への直接アクセスは拒否される |
| F-4-2 | 管理者が自然言語で質問すると、回答が根拠報告書リンク付きでストリーミング表示される（F-2 の画面提供） |
| F-4-3 | 管理者は生成ドラフトを編集・保存し、承認で確定できる。承認後は編集できず、確定版は PDF 出力できる。承認操作は監査ログに残る |
| F-4-4 | 確信度の低い報告書が未分類キューに表示され、管理者が分類を上書きすると「人間確認済み」となりキューから外れる。上書き操作は監査ログに残る |
